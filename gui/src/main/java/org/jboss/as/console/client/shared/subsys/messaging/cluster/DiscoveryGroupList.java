...
public class DiscoveryGroupList {
    ...
    private DefaultCellTable<DiscoveryGroup> factoryTable;
    private ListDataProvider<DiscoveryGroup> factoryProvider;
    ...
    Widget asWidget() {
        ...
        factoryTable = new DefaultCellTable<DiscoveryGroup>(10, new ProvidesKey<DiscoveryGroup>() {
            @Override
            public Object getKey(DiscoveryGroup DiscoveryGroup) {
                return DiscoveryGroup.getName();
            }
        });

        factoryProvider = new ListDataProvider<DiscoveryGroup>();
        factoryProvider.addDataDisplay(factoryTable);

        Column<DiscoveryGroup, String> nameColumn = new Column<DiscoveryGroup, String>(new TextCell()) {
            @Override
            public String getValue(DiscoveryGroup object) {
        ...
        // defaultAttributes
        defaultAttributes = new DiscoveryGroupForm(new FormToolStrip.FormCallback<DiscoveryGroup>() {
            @Override
        ...
        tools.addToolButtonRight(
                new ToolButton(Console.CONSTANTS.common_label_add(), new ClickHandler() {
                    ...
                    public void onClick(ClickEvent clickEvent) {
                        presenter.launchNewDiscoveryGroupWizard();
                    }
                }));
    ...
    public void setDiscoveryGroups(List<DiscoveryGroup> DiscoveryGroups) {
